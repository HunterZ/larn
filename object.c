...
lookforobject(do_ident, do_pickup, do_action)
	...
	case OTRAPDOOR:     
	    ...
	    beep();  losehp(i);  nap(2000);
	    newcavelevel(level+1);  draws(0,MAXX,0,MAXY);  bot_linex();
	    return;
	...
	default:
		...
		switch(i)
		    ...
		    case OCUBEofUNDEAD: case ONOTHEFT:  
			break;
		...
		if (tempc == 't')
		    {   
		    lprcat("take");  if (take(i,j)==0) forget();   
		    return; 
		    }
		ignore();
		}
...
/*
    ...
    Also used to perform the action of a potion without quaffing a potion
    (see invisible capability when drinking from a fountain).
*/
quaffpotion(pot,set_known)
    ...
    /* if player is to know this potion (really quaffing one), make it known */
    if (set_known)
        potionname[pot][0] = ' ';
...
ohome()
    ...
    for (i=0; i<26; i++) if (iven[i]==OPOTION) if (ivenarg[i]==21)
        ...
        if (gtime>TIMELIMIT)
            {
# ifdef MSDOS
    /* The original is a little too morbid for my taste.
     */
...
/*
    ...
    Takes one parameter: if it is negative, going down the shaft,
...
o_open_door()
    ...
    i=0; while ((i!='c') && (i!='i') && (i!='\33')) i=ttgetch();
    if ((i=='\33') || (i=='i')) 
        { 
        ignore();  
        return; 
...
o_closed_door()
    ...
    i=0; while ((i!='o') && (i!='i') && (i!='\33')) i=ttgetch();
    if ((i=='\33') || (i=='i'))
       { ignore();  playerx = lastpx;
         playery = lastpy; return; }
    else
        {
        lprcat("open");
